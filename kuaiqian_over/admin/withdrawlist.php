        ...
        $("#check_all").click(function(){
	   	...
	   	$("#delete").click(function(){
	   		...
	   		if(pdata === ''){
	   	       ...
	   	       $.post("doDelete.php",{id:pdata,tb:"mt4_withdraw"},
	   			function(data){
	   				var msg ="";
	   				if(data == "1"){
                        msg="删除成功！";
...
$ch_name = isset($_REQUEST ['ch_name']) ? trim($_REQUEST ['ch_name']) : '';
...
$r = $mysql->getOne("select count(1) as num from mt4_withdraw $condition");
$totalCount = $r ['num'];
...
$sql = "select * from mt4_withdraw $condition order by time desc limit $p , $pageSize";
...
<div class="container-fluid">
    ...
    <form class="form-inline" id="form_search" method="get">
        <label>交易帐号：</label><input type="text" class="input-small"
                                   id="account" name="account" placeholder="交易帐号"> <label>收款人姓名：</label><input
                                   type="text" id="ch_name" name="ch_name" class="input-small"
                                   placeholder="收款人姓名"> <label class="text"> 时间范围：
...
if (!empty($list)) {
    foreach ($list as $v) {
        ?>
                <tr
                <?php echo ($v['is_success'] == 0 || $v['cjhl'] == 0) ? 'class="error"' : 'class="success"' ?>>
